...
import example.FiberSample.{State, consumer, producer}
...
object FiberSample {
  ...
  case class State[F[_], A](queue: Queue[A], tracker: Queue[Deferred[F, A]])
  ...
  def producer[F[_] : Sync : Console](id: Int, counterR: Ref[F, Int], stateR: Ref[F, State[F, Int]]): F[Unit] = {
    ...
    for {
      ...
      _ <- offer(i)
      _ <- if (i % 10000 == 0) Console[F].println(s"Producer $id hase reached $i items") else Sync[F].unit
  ...
  def consumer[F[_] : Async : Console](id: Int, stateR: Ref[F, State[F, Int]]): F[Unit] = {
    ...
    for {
      i <- take
      _ <- if (i % 10000 == 0) Console[F].println(s"  Consumed $id has reached $i items") else Async[F].unit
